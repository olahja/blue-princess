...
<div class="car-container">
    ...
    <div class="character-container">
        {#each images as image}
            ...
            <figure
                class:selected={selectedCharacter == image.key}
                onclick={(e) => handleCharacterClick(e, image.key)}
            >
                <img src={image.src} alt={image.alt} />
                <figcaption>
                    <h2>{characterDescriptions[image.key].name}</h2>
                    <span style:font-style="italic"
                        >{characterDescriptions[image.key].desc}</span
                    >
                </figcaption>
            </figure>
        {/each}
    ...
    <div class="button-container">
        <button
            ...
            onclick={() => confirmCallback(selectedCharacter)}>Confirm</button
        >
    </div>
</div>
